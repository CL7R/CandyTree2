...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    self.frame=CGRectMake(-140, 87, 100, 100);
...
{
    //CL7R modify
    CLog(@"[quadCurveMenuItemTouchesBegan],%d,%d",item.tag,self.isExpanding);
    
    if (item == _addButton) 
    {
        if (self.isExpanding==0) {
            [UIView beginAnimations:@"Animation"context:nil];
            [UIView setAnimationDuration:0.3];
            self.frame=CGRectMake(0, -95, 100, 100);
            [UIView commitAnimations];
        }
        else{
            [UIView beginAnimations:@"Animation"context:nil];
